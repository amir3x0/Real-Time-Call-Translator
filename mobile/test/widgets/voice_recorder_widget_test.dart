...
void main() {
  testWidgets('VoiceRecorderWidget shows upload button after recording', (WidgetTester tester) async {
    // Give the widget a constrained size to avoid overflow in tests
    await tester.pumpWidget(const MaterialApp(home: Scaffold(body: SingleChildScrollView(child: SizedBox(width: 600, height: 800, child: VoiceRecorderWidget())))));
    ...
    expect(find.descendant(of: find.byKey(const Key('voice-upload-button')), matching: find.text('Upload')), findsOneWidget);
  });
}
